...
function formatText(elem, options) {
	...
	if (options.isInPre) {
		...
		//return helper.wordwrap(text, options);
	}
}
...
function formatImage(elem, options) {
	...
	if (attribs.alt) {
		result += _s.strip(he.decode(attribs.alt, options.decodeOptions));
	}
...
function formatParagraph(elem, fn, options) {
	options.inP = true;
	var base = fn(elem.children, options);
	if (!options.aFlag) {
		var ret = base + '\n\n';
		options.inP = false;
	} else if (options.aFlag) {
		var ret = base + '\n>> ' + options.linkstore + '\n\n';
		options.aFlag = false;
		options.inP = false;
	}
...
function formatAnchor(elem, fn, options) {
	...
	if (!options.ignoreHref) {
		...
		if (href) {
			...
			if (options.inP) {
				options.aFlag = true;
				options.linkstore = href;
				result += '';
			} else if (!options.hideLinkHrefIfSameAsText || href != _s.replaceAll(result, '\n', '')) {
				result += '';
			}
		}
	}
	...
	result += '';
	result = ' ' + result;
//	console.log(clc.magenta(result));
